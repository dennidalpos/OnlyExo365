...
The worker depends on `Microsoft.PowerShell.SDK` `7.6.1`, while the product ships as a Windows desktop shell plus a separate PowerShell-backed worker. The repository also enforces packaging, smoke validation, signing-flow validation, and release automation around that split runtime model.
...
- `Microsoft.PowerShell.SDK` `7.6.1` in the worker
