...
public class IpcClient : IAsyncDisposable
{
    ...
    public IpcClient(IpcSessionContext sessionContext, string sessionToken)
    {
        _sessionContext = sessionContext ?? throw new ArgumentNullException(nameof(sessionContext));
        _sessionToken = string.IsNullOrWhiteSpace(sessionToken)
    ...
    public async Task<HandshakeResponse> ConnectAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            _requestPipe = new NamedPipeClientStream(".", _sessionContext.RequestPipeName, PipeDirection.InOut, PipeOptions.Asynchronous | PipeOptions.CurrentUserOnly);
            _eventPipe = new NamedPipeClientStream(".", _sessionContext.EventPipeName, PipeDirection.In, PipeOptions.Asynchronous | PipeOptions.CurrentUserOnly);
            ...
            var handshakeRequest = new HandshakeRequest
            {
                ...
                UserScope = _sessionContext.UserScope
            };
            ...
            if (!response.SessionValidated)
            {
                throw new InvalidOperationException("Handshake failed: worker did not validate session binding");
            }
