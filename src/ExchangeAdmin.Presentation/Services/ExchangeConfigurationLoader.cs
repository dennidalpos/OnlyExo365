...
public static class ExchangeConfigurationLoader
{
    ...
    private const string LegacyProductDirectoryName = "ExchangeAdmin";
    ...
    internal static string GetLegacyPerMachineConfigurationDirectoryPath()
    {
        return Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData),
            VendorDirectoryName,
    ...
    private static IEnumerable<string> EnumerateSharedConfigurationDirectories(string sharedConfigurationDirectory)
    {
        ...
        var parentDirectory = Directory.GetParent(sharedConfigurationDirectory)?.FullName;
        var legacyDirectory = string.IsNullOrWhiteSpace(parentDirectory)
            ? null
            : Path.Combine(parentDirectory, LegacyProductDirectoryName);

        if (!string.IsNullOrWhiteSpace(legacyDirectory) &&
            !string.Equals(legacyDirectory, sharedConfigurationDirectory, StringComparison.OrdinalIgnoreCase))
        {
