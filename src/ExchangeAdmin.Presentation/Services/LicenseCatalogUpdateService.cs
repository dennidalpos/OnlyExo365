...
public sealed class LicenseCatalogUpdateService : IDisposable
{
    ...
    private const string EmbeddedWorkerCatalogResourceName =
        "ExchangeAdmin.Worker.Data.Microsoft365SkuCatalog.json";
    ...
    private static Assembly? TryGetWorkerAssembly()
    {
        ...
        var loaded = AppDomain.CurrentDomain.GetAssemblies()
            .FirstOrDefault(a => a.GetName().Name == "ExchangeAdmin.Worker");
        if (loaded != null)
        {
            return loaded;
        }
        ...
        var workerPath = Path.Combine(baseDir, "ExchangeAdmin.Worker.dll");
