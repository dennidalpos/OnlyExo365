...
public sealed class LogsViewModel : ViewModelBase, IDisposable
{
    ...
    private readonly string _persistentLogDirectory;
    ...
    public LogsViewModel(ShellViewModel shellViewModel)
    {
        ...
        _persistentLogDirectory = PersistentLogWriter.GetDefaultLogDirectoryPath();
        _defaultExportDirectory = ExcelExportService.ResolveExportDirectory();
        ...
        ExportPersistentLogsCommand = new RelayCommand(ExportPersistentLogs);
    ...
    public string PersistentLogDirectory => _persistentLogDirectory;
    public string PersistentObservabilitySummaryPath => PersistentLogStore.GetSummaryFilePath(_persistentLogDirectory);

    public string PersistentLogPolicySummary => $"Persisted JSONL logs retained for {PersistentLogWriter.ResolveRetentionDays()} UTC day(s).";
    public string PersistentObservabilitySummary => "Collector-friendly summary JSON is refreshed from persisted logs and included in exports.";
    ...
    public ICommand ExportPersistentLogsCommand { get; }
    ...
    private void RebuildFilterCache()
    {
        ...
        foreach (var log in LogEntries)
        {
            ...
            if (!string.IsNullOrEmpty(searchLower))
            {
                ...
                var correlationContains = log.CorrelationId?.ToLowerInvariant().Contains(searchLower) ?? false;

                if (!messageContains && !sourceContains && !correlationContains)
                {
                    continue;
                }
            }
    ...
    private void CopyLogs()
    {
        ...
        var lines = logs.Select(l => $"[{l.Timestamp:HH:mm:ss.fff}] [{l.Level}] [{l.Source}] [corr={l.CorrelationDisplay}] {l.Message}");
    ...
    private void ExportPersistentLogs()
    {
        try
        {
            PersistentLogStore.ApplyRetention(
                _persistentLogDirectory,
                DateTime.UtcNow,
                PersistentLogWriter.ResolveRetentionDays());

            var persistedFiles = PersistentLogStore.GetLogFiles(_persistentLogDirectory);
            if (persistedFiles.Count == 0)
            {
                _shellViewModel.AddLog(LogLevel.Warning, $"No persisted log files found in {_persistentLogDirectory}");
                return;
            }
            ...
            var dialog = new SaveFileDialog
            {
                ...
                FileName = $"exchangeadmin-logs-{DateTime.Now:yyyyMMdd-HHmmss}.zip",
            ...
            var exportedCount = PersistentLogStore.ExportLogArchive(dialog.FileName, _persistentLogDirectory);
            _shellViewModel.AddLog(
                LogLevel.Information,
                $"Exported {exportedCount} persisted log file(s) to {dialog.FileName}");
        }
        catch (Exception ex)
        {
            _shellViewModel.AddLog(LogLevel.Error, $"Failed to export persisted logs: {ex.Message}");
        }
    }
