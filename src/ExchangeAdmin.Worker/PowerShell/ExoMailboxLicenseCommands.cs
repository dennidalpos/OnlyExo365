...
";
        ...
        if (!_engine.IsGraphConnected)
        {
            throw new InvalidOperationException(
                ...
                (requireLicenseWriteScope
                    ? $"Microsoft Graph connection failed. Configure EXCHANGEADMIN_GRAPH_LICENSE_WRITE_SCOPES with {LicenseWriteScope} to enable license assignment changes."
                    : "Microsoft Graph connection failed. Configure EXCHANGEADMIN_GRAPH_SCOPES with the minimum scopes required for the requested Graph operation."));
        }
    }
    ...
    private IReadOnlyList<string> GetLicenseWriteScopesOrThrow()
    {
        ...
        {
            throw new InvalidOperationException(
                $"Mailbox licensing write operations are disabled by configuration. Add {LicenseWriteScope} to graphLicenseWriteScopes or EXCHANGEADMIN_GRAPH_LICENSE_WRITE_SCOPES to enable Set-MgUserLicense.");
        }
