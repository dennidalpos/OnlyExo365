...
namespace OnlyExo365.Shell.Resilience;
...
public class RetryPolicy
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        while (attempt < _options.MaxRetries)
        {
            ...
            try
            {
                Debug.WriteLine($"[RetryPolicy] Result attempt {attempt}/{_options.MaxRetries}");
                var result = await operation(cancellationToken).ConfigureAwait(false);

                if (result.IsSuccess || result.WasCancelled)
                {
                    Debug.WriteLine($"[RetryPolicy] Result success/cancelled on attempt {attempt}");
                    return result;
                }

                lastResult = result;
                var error = result.Error!;
                Debug.WriteLine($"[RetryPolicy] Result attempt {attempt} failed: {error.Code} - {error.Message}");

                if (NonRetryableErrors.IsNonRetryable(error.Code))
                {
                    Debug.WriteLine($"[RetryPolicy] Result error {error.Code} is non-retryable, stopping");
                    return result;
                }

                if (!error.IsTransient)
                {
                    Debug.WriteLine("[RetryPolicy] Result error is not transient, stopping");
                    return result;
                }

                if (attempt >= _options.MaxRetries)
                {
                    Debug.WriteLine($"[RetryPolicy] Result max retries ({_options.MaxRetries}) exhausted");
                    return result;
                }

                var delay = CalculateDelay(attempt, error.RetryAfter, ref previousDelay);
                Debug.WriteLine($"[RetryPolicy] Waiting {delay.TotalMilliseconds:F0}ms before result retry {attempt + 1}");
                await Task.Delay(delay, cancellationToken).ConfigureAwait(false);
            }
            catch (OperationCanceledException)
            {
                Debug.WriteLine($"[RetryPolicy] Result cancelled on attempt {attempt}");
                return Result<T>.Cancelled();
            }
            catch (Exception ex)
            {
                var error = NormalizedError.FromException(ex);
                Debug.WriteLine($"[RetryPolicy] Result attempt {attempt} threw: {error.Code} - {error.Message}");

                if (NonRetryableErrors.IsNonRetryable(error.Code) || !error.IsTransient || attempt >= _options.MaxRetries)
                {
                    return Result<T>.Failure(error);
                }

                var delay = CalculateDelay(attempt, error.RetryAfter, ref previousDelay);
                Debug.WriteLine($"[RetryPolicy] Waiting {delay.TotalMilliseconds:F0}ms before retry after exception {attempt + 1}");
                await Task.Delay(delay, cancellationToken).ConfigureAwait(false);
            }
        }

        return lastResult ?? Result<T>.Failure(NormalizedError.Create(
            ErrorCode.Unknown,
            $"Retry policy exited without a result after {_timeProvider.UtcNow - startTime}.",
            isTransient: false));
    }

    public async Task<Result> ExecuteResultAsync(
        Func<CancellationToken, Task<Result>> operation,
        CancellationToken cancellationToken = default)
    {
        var typedResult = await ExecuteResultAsync(async ct =>
        {
        ...
        return typedResult.IsSuccess
            ? Result.Success()
            : typedResult.WasCancelled
                ? Result.Cancelled()
                : Result.Failure(typedResult.Error!);
    }
