...
public sealed class BuildScriptSecurityTests
{
    ...
    public void BuildScript_StopsArtifactProcessesBeforeRetryingLockedArtifactsCleanup()
    {
        ...
        Assert.Contains("function Get-ArtifactExchangeAdminProcesses", script, StringComparison.Ordinal);
        Assert.Contains("function Stop-ArtifactExchangeAdminProcesses", script, StringComparison.Ordinal);
        Assert.Contains("Detected running ExchangeAdmin process(es) from artifacts", script, StringComparison.Ordinal);
    ...
    public void PackAndPublishScripts_AreExeOnly()
    {
        ...
        Assert.DoesNotContain("ExchangeAdmin.msi", packScript, StringComparison.Ordinal);
    ...
    public void LegacyCreateMsiEntryPoint_HasBeenRemovedFromRepository()
    {
        ...
        var wixAuthoringPath = GetRepositoryFilePath("installer", "ExchangeAdmin.wxs");
    ...
    {
        var scriptPath = GetRepositoryFilePath("installer", "ExchangeAdmin.iss");
        var script = File.ReadAllText(scriptPath);

        Assert.Contains("AppId={{A8C31B9E-49D2-4A23-9F57-7A39B3E5BCA2}", script, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("ArchitecturesAllowed=x64compatible", script, StringComparison.Ordinal);
        Assert.Contains("Source: \"{#PublishDirX64}\\*\"", script, StringComparison.Ordinal);
        Assert.Contains("Source: \"{#PublishDirX86}\\*\"", script, StringComparison.Ordinal);
        Assert.Contains("Check: Is64BitInstallMode", script, StringComparison.Ordinal);
        Assert.Contains("Check: not Is64BitInstallMode", script, StringComparison.Ordinal);
        ...
        Assert.Contains("ExchangeAdmin.ServiceCleanup.ps1", script, StringComparison.Ordinal);
    ...
    public void SmokeTestScripts_BackUpAndRestorePreExistingLocalDataBeforeInstallerValidation()
    {
        ...
        Assert.Contains("Restored pre-existing ExchangeAdmin local data after installer validation.", combined, StringComparison.Ordinal);
        Assert.Contains("Recovered pre-existing ExchangeAdmin local data during final cleanup after an interrupted installer smoke test run.", combined, StringComparison.Ordinal);
        Assert.DoesNotContain("artifacts/packages/ExchangeAdmin.msi", script, StringComparison.Ordinal);
    }

    [Fact]
    public void PowerShellBootstrapPolicy_PinsApprovedModuleVersions()
    {
        var policyPath = GetRepositoryFilePath("src", "ExchangeAdmin.Worker", "Data", "PowerShellModuleBootstrapPolicy.json");
    ...
    {
        var scriptPath = GetRepositoryFilePath("src", "ExchangeAdmin.Worker", "PowerShell", "ExoSupportCommands.cs");
        var script = File.ReadAllText(scriptPath);
    ...
    public void FallbackAndManualInstructionText_RemainsEnglishInBaselinePaths()
    {
        foreach (var relativePath in new[]
                 {
                     new[] { "src", "ExchangeAdmin.Presentation", "ViewModels", "MigrationViewModel.BatchCreation.cs" },
                     new[] { "src", "ExchangeAdmin.Worker", "PowerShell", "ExoSupportCommands.cs" },
                     new[] { "src", "ExchangeAdmin.Worker", "PowerShell", "PowerShellModuleBootstrapPolicy.cs" }
                 })
        {
    ...
    public void CreateSetupExeScript_UsesInnoSetupCompilerAndInstallerAuthoring()
    {
        ...
        Assert.Contains("installer\\ExchangeAdmin.iss", script, StringComparison.Ordinal);
        Assert.Contains("Get-InnoSetupCompilerPath -RepositoryRoot $repositoryRoot", script, StringComparison.Ordinal);
        Assert.Contains("/DPublishDirX64=$resolvedPublishPathX64", script, StringComparison.Ordinal);
        Assert.Contains("/DPublishDirX86=$resolvedPublishPathX86", script, StringComparison.Ordinal);
    ...
    public void CanonicalScripts_EnforcePinnedDotNetSdkAndCanonicalArtifactPaths()
    {
        ...
        Assert.Contains("OnlyExo365.Setup.exe", packScript, StringComparison.Ordinal);
        Assert.DoesNotContain("ExchangeAdmin.msi", packScript, StringComparison.Ordinal);
        Assert.Contains("[string]$RuntimeIdentifier = \"multi-arch\"", publishScript, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("artifacts/packages/ExchangeAdmin.msi", publishScript, StringComparison.Ordinal);
    ...
    public void ReleaseValidationWorkflow_RunsExeOnlyPackagingAndSmokeValidation()
    {
        ...
        Assert.DoesNotContain("ExchangeAdmin.msi", workflow, StringComparison.Ordinal);
    }
    ...
    public void WindowsReleaseBaselineCompositeAction_RunsCanonicalPreSigningValidationAndPackaging()
    {
        ...
        Assert.Contains("./build/assert-no-vulnerable-packages.ps1 -SolutionPath ExchangeAdmin.sln -ReportPath artifacts/security/nuget-vulnerabilities.json", action, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("ExchangeAdmin.msi", action, StringComparison.Ordinal);
    }
    ...
    public void TenantValidationWorkflow_UsesControlledAppCertificateValidationAndPublishesReport()
    {
        ...
        Assert.Contains("EXCHANGEADMIN_CERTIFICATE_PFX_BASE64", workflow, StringComparison.Ordinal);
        ...
        Assert.Contains("EXCHANGEADMIN_AUTH_MODE: AppCertificate", workflow, StringComparison.Ordinal);
    }
