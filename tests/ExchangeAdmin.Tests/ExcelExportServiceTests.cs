...
public sealed class ExcelExportServiceTests
{
    ...
    {
        var previousValue = Environment.GetEnvironmentVariable("EXCHANGEADMIN_EXPORT_DIR");
        try
        {
            Environment.SetEnvironmentVariable("EXCHANGEADMIN_EXPORT_DIR", " C:\\exports\\onlyexo365 ");
        ...
        {
            Environment.SetEnvironmentVariable("EXCHANGEADMIN_EXPORT_DIR", previousValue);
        }
    }
    ...
    {
        var tempFile = Path.Combine(Path.GetTempPath(), $"exchangeadmin-export-{Guid.NewGuid():N}.xlsx");
