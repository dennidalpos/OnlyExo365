...
public sealed class LicenseCatalogConfigurationTests : IDisposable
{
    ...
    [Fact]
    public void LoadLicenseCatalogConfiguration_UsesLegacySharedDirectory_WhenCurrentDirectoryIsEmpty()
    {
        var currentSharedDirectory = Path.Combine(_tempDir, "OnlyExo365");
        var legacySharedDirectory = Path.Combine(_tempDir, "ExchangeAdmin");
        ...
        var config = ExchangeConfigurationLoader.LoadLicenseCatalogConfiguration(
            _tempDir, currentSharedDirectory);
