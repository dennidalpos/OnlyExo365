...
public class WorkerSupervisorTests
{
    ...
    public void ResolveWorkerPath_UsesBaseDirectoryForRelativeConfiguredPath()
    {
        ...
        var resolvedPath = WorkerSupervisor.ResolveWorkerPath(
            configuredWorkerPath: Path.Combine("workers", "ExchangeAdmin.Worker.exe"),
            baseDirectory: baseDirectory);

        Assert.Equal(
            Path.GetFullPath(Path.Combine(baseDirectory, "workers", "ExchangeAdmin.Worker.exe")),
            resolvedPath);
    }
    ...
    public void ResolveWorkerPath_UsesDefaultWorkerExecutableNameWhenConfigurationIsMissing()
    {
        ...
        Assert.Equal(
            Path.GetFullPath(Path.Combine(baseDirectory, "ExchangeAdmin.Worker.exe")),
            resolvedPath);
    }
